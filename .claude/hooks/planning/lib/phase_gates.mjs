...
export function nextActionHint(state) {
  ...
  switch (String(cp)) {
    case "0": return "Keep pre-flight bounded: resolve target with the existing resolver, run index.sh, verify required tools/dependencies and target-scoped history/<feature>/, collect terminal success, then immediately enter Phase 1.";
    case "1": return "Immediately launch missing/failed/orphaned Architecture/Patterns/Constraints/External lanes as background general-purpose agents using the canonical versioned prompt block; do not pre-mark running, and treat identity-less running as orphaned/retryable. Then read/verify the four canonical files, fill only specific gaps, and compile discovery.md.";
