...
export async function validateSessionStart(input, projectDir) {
  ...
  const resumeRequirement = phase === "1"
    ? `  resume requirement: derive HISTORY_ROOT from phase_outputs.0.project_index_root, then if the External lane is missing/failed/orphaned, launch it first as a background general-purpose agent — exactly one Agent call in its own message. Copy the exact ${DISCOVERY_CONTRACT_BEGIN} block from launch-discovery-agents.md and substitute the actual feature/artifact. The Architecture, Patterns, and Constraints lanes are main-agent-owned: produce any missing 1-architecture.md / 2-patterns.md / 3-constraints.md directly with GitNexus/Serena instead of spawning subagents. Do not pre-mark running; running requires verified launch identity. Identity-less running is orphaned/retryable. Once all four files exist, read them as default context and self-fill only specific gaps.`
  ...
  const lines = [
    ...
    resumeRequirement,
    "  execution style: after Phase 1.5 reaches 12/12, continue into Phase 1.6 for 8/8 test clarification, then continue in one run through Phase 2 into Phase 2.5; if Phase 2.5 is approved, continue in one run through Phase 3 and Phase 4, pause only at the Phase 4 approval AskUserQuestion, and after exact Approve continue immediately in one run through Phase 5 (materialize_beads.mjs) and Phase 6, then stop planning when Phase 6 records cycles_found=0 and planning_active=false.",
