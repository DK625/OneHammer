...
export async function validateUserPromptSubmit(input, projectDir) {
  ...
  if (earlyIndex.status === "started") {
    lines.push(
      ...
      `After a successful wait and health checks, record Phase 0 evidence, set current_phase=1, and immediately spawn all four missing discovery lanes as background general-purpose agents using the exact ${DISCOVERY_CONTRACT_BEGIN} block from launch-discovery-agents.md before broad main-agent exploration. Do not pre-mark lanes running; record running only after accepted launch with verified identity.`,
    );
  ...
  if (active) {
    ...
    const resumeLine = activePhase === "1"
      ? `Resume requirement for Phase 1: derive HISTORY_ROOT from Phase 0 state and immediately launch missing/failed/orphaned discovery lanes as background general-purpose agents before broad main-agent context reads. Copy the exact ${DISCOVERY_CONTRACT_BEGIN} block from launch-discovery-agents.md. Do not pre-mark running; running without verified launch identity is orphaned/retryable. After all four files exist, read them as default context and fill only specific gaps.`
