...
export async function validateUserPromptSubmit(input, projectDir) {
  ...
  if (earlyIndex.status === "started") {
    lines.push(
      ...
      `After a successful wait and health checks, record Phase 0 evidence, set current_phase=1, and immediately spawn the three missing subagent discovery lanes (Patterns, Constraints, External) as background general-purpose agents — one lane per message, exactly one Agent call per response — using the exact ${DISCOVERY_CONTRACT_BEGIN} block from launch-discovery-agents.md; then run the Architecture lane in the main agent itself with GitNexus (query/context/impact/route_map/cypher) and write 1-architecture.md directly — never spawn an Architecture subagent. Do not pre-mark lanes running; record running only after accepted launch with verified identity.`,
    );
  ...
  if (active) {
    ...
    const resumeLine = activePhase === "1"
      ? `Resume requirement for Phase 1: derive HISTORY_ROOT from Phase 0 state and immediately launch missing/failed/orphaned subagent lanes (Patterns, Constraints, External) as background general-purpose agents, one lane per message. Copy the exact ${DISCOVERY_CONTRACT_BEGIN} block from launch-discovery-agents.md. If 1-architecture.md is missing, produce it directly in the main agent with GitNexus instead of spawning a subagent. Do not pre-mark running; running without verified launch identity is orphaned/retryable. After all four files exist, read them as default context and fill only specific gaps.`
