...
function phase1AgentGuidance(message) {
  return additionalContext(
    "PostToolBatch",
    `[planning-guard] ${message}\n\nCorrective action: do not ask the user to fix this manually. Copy the exact ${DISCOVERY_CONTRACT_BEGIN} block from .claude/skills/planning/references/launch-discovery-agents.md, substitute the actual feature/lane artifact, and re-issue exactly one Agent call in its own message for a missing/failed/orphaned External lane only; multi-call batches get truncated and drop subagent_type/run_in_background or cut the contract block. The Architecture, Patterns, and Constraints lanes are main-agent-owned (GitNexus/Serena-direct) — never spawn subagents for them. The Architecture lane is main-agent-owned: run GitNexus directly and write 1-architecture.md yourself instead of spawning it. Do not pre-mark running. Record running only after an accepted launch with a verified launch identity. Canonical lane files are the handoff; the main agent compiles discovery.md and manages JSON state.`,
  );
}
