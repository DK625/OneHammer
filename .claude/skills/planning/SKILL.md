...
5. Immediately after successful Phase 0, launch the four missing/retryable discovery lanes with background `Agent` calls using `subagent_type="general-purpose"` and the exact versioned `[PLANNING_DISCOVERY_AGENT_CONTRACT_V1]` block from `agents/launch-discovery-agents.md`; do not rely on fragile paraphrase. Each lane agent writes its own full detailed, non-summary canonical Markdown file directly under `HISTORY_ROOT/history/<feature>/discovery-lanes/`. The main agent verifies/reads those files, compiles `discovery.md`, and manages planning state; it must not copy response bodies into lane files or replace lane files with summaries.
...
|---|---|---|---|
| 0 | Fast bounded pre-flight: resolve target, run existing resolver/index entrypoints, verify required tools/dependencies, collect index result | target-repo `history/<feature>/` + JSON state evidence | safe target resolved first; `resolve_index_root.mjs` + `index.sh --target <repo>` are the canonical scripts; `index.sh` verifies/runs Serena + GitNexus indexing; `.mcp.json`/Serena and `br`/`bv`/`jq` health checks pass; no broad requirement/code/docs read; mandatory wait/collect exits 0 before completion |
| 1 | Discover feature context | 4 lane-owned files + `discovery.md` | immediately spawn all missing/failed/orphaned `general-purpose` lanes after Phase 0 using the canonical versioned prompt block; never pre-mark `running`; main reads/verifies files, fills only specific gaps, then compiles `discovery.md` |
...
Use `references/pre-flight.md`. **Ordering is mandatory and bounded:** on `/planning`, derive target intent from the current prompt/path, call the existing `resolve_index_root.mjs` resolver, and start the canonical `.claude/hooks/planning/index.sh --target <resolved-root> --background` entrypoint before broad context work. `UserPromptSubmit` may already have started that job; reuse its job id instead of launching a duplicate. While indexing runs, perform only Phase 0 health checks and evidence work: required MCP config/runtime readiness, required CLI checks, target-scoped workspace creation, and minimal authoritative-state bookkeeping. Do **not** read broad requirement/code/project docs in Phase 0. Before Phase 0 completes, run `index.sh --wait --job <id>` and collect terminal evidence. Any non-zero exit is fail-closed: stop planning immediately, report the indexing error, keep Phase 0 incomplete, and do not continue to discovery. After success, transition immediately to Phase 1 and spawn the four discovery lanes before any broad main-agent exploration.
...
Use `agents/launch-discovery-agents.md`. The 4 lanes are orthogonal dimensions, not backend/frontend splits. **Immediately after Phase 0 succeeds**, spawn all four missing lanes in parallel before the main agent broad-reads requirement context, wanders through code/docs, or performs its own discovery pass:
