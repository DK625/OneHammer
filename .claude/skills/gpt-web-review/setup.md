...
```powershell
scp C:\Users\noname\.oracle\bridge-connection.json openclaw:~/bridge-connection.json
```
...
**Terminal 2:**
```powershell
ssh -N -R 9473:127.0.0.1:9473 openclaw
```
...
```powershell
ssh -R 9473:localhost:9473 openclaw -N  # SAI — localhost resolve thành ::1 (IPv6) trên Windows
```
...
**Fix:**
```powershell
ssh -R 9473:127.0.0.1:9473 openclaw -N  # ĐÚNG — force IPv4
```
...
Trường hợp đã gặp: chạy `oracle serve --port 9473` làm Oracle listen ở IP LAN như `openclaw:9473`, trong khi SSH tunnel forward về `127.0.0.1:9473`.
...
```powershell
oracle bridge host --bind 127.0.0.1:9473 --token f667af3845a136e7a1d6573a1d0ecff2 --ssh openclaw --ssh-remote-port 9474 --ssh-extra-args "-o ExitOnForwardFailure=yes" --foreground --print
```
...
2. Windows: oracle bridge host --bind 127.0.0.1:9473 --token f667af3845a136e7a1d6573a1d0ecff2 --ssh openclaw --ssh-remote-port 9473 --ssh-extra-args "-o ExitOnForwardFailure=yes" --foreground --print
